...
const int ipnum_sems = 1; 	         /* number of semaphores in set */
...
static int InitSem(void)
{
	...
	if ((sem_key1 == -1)||(sem_key2 == -1))
	{
		...
		snprintf(logString,256,"[ERROR] ipUpdate.c @ InitSem @ ftok --- %s", strerror(errno));
		goto err_out;
	}

	sem_send = semget(sem_key1, ipnum_sems, IPC_CREAT|0660);
	sem_recv = semget(sem_key2, ipnum_sems, IPC_CREAT|0660);
	if ((sem_send == -1)||(sem_recv == -1))
	{
		memset(logString, 0, 256);
		snprintf(logString,256,"[ERROR] ipUpdate.c @ InitSem @ semget --- %s", strerror(errno));
		goto err_out;
	}
	...
	{
		memset(logString, 0, 256);
		snprintf(logString,256,"[ERROR] ipUpdate.c @ InitSem @ semctl --- %s", strerror(errno));
		goto err_out;
	}
	sem_union.val = 0;
...
void* IPUpdateCreate(void *arg)
{
	...
	while(1)
	{
		...
		if(ret == -1)
		{
			...
			if(retry < 3)
			{
				snprintf(logString,256,"[WARNING] ipUpdate.c @ UpdateCreate @ semaphore_p --- semaphore maybe destory, try resume it!");
				Logging(PROG_ERROR_LOG, logString);
				IPInitShm();	// 尝试重新创建信号量及共享内存
				retry++;
				continue;
			}else{
				snprintf(logString,256,"[ERROR] ipUpdate.c @ UpdateCreate @ semaphore_p --- semaphore maybe destory, resume failed!");
				Logging(PROG_ERROR_LOG, logString);
				return (void*)(-11);
			}
		}
		...
		{
			memset(logString, 0, 256);
			snprintf(logString,256,"[WARNING] ipUpdate.c @ UpdateCreate @ semaphore_v --- semaphore maybe destory, try resume it!");
			Logging(PROG_ERROR_LOG, logString);
			return (void*)(-12);
		}
	}
...
static void ProcessUpdate(void *buffer)
{
    ...
    if(recdhdr->data_type==DATA_TYPE_IP)
	{
	    printf("Find IP\n");
		gettimeofday (&t1,&tz);
		sum=UpdateIpdata(buffer+sizeof(*recdhdr), recdhdr->recd_size,recdhdr->update_type);
		printf("Total:%d\n",sum);
		gettimeofday (&t2,&tz);
		update_t = (t2.tv_sec-t1.tv_sec)*1000000+(t2.tv_usec-t1.tv_usec);

		memset(logString, 0, 256);
		snprintf(logString,256,"Type: IP.  Counts: %u.  Update-Type: NORMAL\n \
		    	   Time-Valid:  %lu us   Time-Total:  %lu us\n", sum, update_t, update_t);

		Logging(PROG_UPDATE_LOG, logString);

       		 //write data to file;
        SaveIPToFile(buffer+sizeof(*recdhdr),recdhdr->recd_size);
		//kill(getpid(), SIGUPDATE);
	}
}
...
result_t IPInitShm(void)
{
	...
	{
		memset(logString, 0, 256);
		snprintf(logString,256,"[ERROR] ipUpdate.c @ InitShm @ ftok --- %s", strerror(errno));
		goto err_out;
	}
	...
	{
		memset(logString, 0, 256);
		snprintf(logString,256,"[ERROR] ipUpdate.c @ InitShm @ shmget --- %s", strerror(errno));
		goto err_out;
	}
	if((share_region=shmat(share_id, 0, 0))==(void *)(-1))
	{
		memset(logString, 0, 256);
		snprintf(logString,256,"[ERROR] ipUpdate.c @ InitShm @ shmat --- %s", strerror(errno));
		goto err_out;
	}
	// 初始化信号量
	if(InitSem() == -1)
